...
def uniform 	(rho, lamb, number):
	return np.sort(np.random.uniform(0, 2 * rho / lamb, number))
def uniform_special 	(a, b, number):
	return np.sort(np.random.uniform(a, b, number))

def exponential (rho, lamb, number):
	return np.sort(np.random.exponential(rho/lamb, number))
def exponential_special (param, number):
	return np.sort(np.random.exponential(param, number))

def triangular 	(rho, lamb, number):
	return np.sort(np.random.triangular(0, 0, 3 * rho / lamb, number))
def triangular_special 	(a, b, c, number):
	if c > a:
		a, c = c, a
	elif c == a:
		c = c + 0.00001
	return np.sort(np.random.triangular(a, b, c, number))
